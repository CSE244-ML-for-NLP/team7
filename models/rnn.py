    ...
    def forward(self, x1, x1_len,bert,app_time=None, embedding=None):
        ...
        if bert:
            with torch.no_grad():
                ...
                xt=x1.transpose(0, 1)
                encoded_layers=embedding(xt, attention_mask=input_mask)   
...
#         self.dp2=nn.Dropout(0.1)
        ...
        # self.rnn1 = nn.LSTM(input_size=4*config.emb_size, hidden_size=config.encoder_hidden_size,
        #                   num_layers=config.num_layers, dropout=0.5, bidirectional=config.bidirec)
    ...
    def forward(self, tgt, init_state, ifslot,contexts,criterion,story,lengths,slot=None):
        ...
        for emb in embs.split(1):
            ...
            outputt1, _ = self.attention(outputt, contexts[2],lengths[2])
            # outputt1=outputt+output1
            outputt2, _ = self.attention(outputt1, contexts[1],lengths[1])
            # outputt2=outputt1+output2
            outputt3, _ = self.attention(outputt2, contexts[0],lengths[0])
            # outputt3=outputt2+output3
...
#             output=self.re4(self.linear4(output))
            ...
            # output, (h, c) = self.rnn1(torch.cat([cout, cout1, cout2, outputt3], 1))
