...
validloader = dataloader.get_loader(validset, batch_size=1, shuffle=False, num_workers=0)
testloader = dataloader.get_loader(testset, batch_size=1, shuffle=False, num_workers=0)
...
def train_and_eval(epoch):
    ...
    for src1, src1_len, src2,src2_len, src3, src3_len, tgt, tgt_len,tgtv, tgtv_len,tgtpv, tgtpv_len in trainloader:
        # logging("loading data time: %6.3f \n\n" % (time.time()-time0))
        ...
        for i in range(src1.shape[1]):
            ...
            # losses=losses/src1.shape[1]
            # total_lossl.append(loss.data/num)
            # total_slossl.append(torch.sum(torch.tensor(sloss)).data/snum) 
            # total_vlossl.append(torch.sum(torch.tensor(vloss)).data/vnum) 
            
            # print(sloss)
            # print(sum(sloss).device)
            
            # print(torch.sum(torch.tensor(sloss)).data/snum, " ", sum(sloss).data/snum)
            
            losses.backward()
            
            # logging("id: %2d, sample time: %6.3f \n\n" % ((i + 1), time.time()-times))
